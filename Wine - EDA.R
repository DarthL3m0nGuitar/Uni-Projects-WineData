...
#---------------------D--------------------------
# Calculate median of residual sugar by wine type and quality
median_table <- train_set %>%
  group_by(type, quality) %>%
  summarise(median_residual_sugar = median(residual.sugar, na.rm = TRUE))
#plotting the boxplot for residual sugar
ggplot(train_set, aes(x = quality, y = residual.sugar, fill = type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("red" = "#D32F2F", "white" = "#FFEB3B")) + 
  labs(x = "Quality", y = "Residual Sugar", fill = "Wine Type") +
  geom_text(data = median_table, aes(x = quality, y = median_residual_sugar, label = round(median_residual_sugar, 2)), 
            position = position_dodge(width = 0.75), vjust = -0.5) +  
  theme_minimal()

#chloride with quality and type
median_table <- train_set %>%
  group_by(type, quality) %>%
  summarise(median_chlorides = median(chlorides, na.rm = TRUE))
#plotting the boxplot for chlorides
ggplot(train_set, aes(x = quality, y =chlorides, fill = type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("red" = "#D32F2F", "white" = "#FFEB3B")) + 
  labs(x = "Quality", y = "chlorides", fill = "Wine Type") +
  geom_text(data = median_table, aes(x = quality, y = median_chlorides, label = round(median_chlorides, 2)), 
            position = position_dodge(width = 0.75), vjust = -0.5) +  
  theme_minimal()

# Calculate median alcohol values for each type and quality level
median_table <- train_set %>%
  group_by(type, quality) %>%
  summarise(median_alcohol = median(alcohol, na.rm = TRUE))
# Plotting the boxplot for alcohol
ggplot(train_set, aes(x = factor(quality), y = alcohol, fill = type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("red" = "#D32F2F", "white" = "#FFEB3B")) + 
  labs(x = "Quality", y = "Alcohol", fill = "Wine Type") +
  geom_text(data = median_table, aes(x = factor(quality), y = median_alcohol, label = round(median_alcohol, 2)), 
            position = position_dodge(width = 0.75), vjust = -0.5) + 
  theme_minimal()
...
# Scatter plot: Alcohol vs Volatile Acidity with Quality
poor_exc <- train_set %>% filter(quality %in% c("Poor", "Excellent"))
ggplot(poor_exc, aes(x = volatile.acidity, y = alcohol, color = quality)) +
  geom_point(alpha = 0.7, size = 3) +
  scale_color_manual(values = c("Poor" = "red","Excellent" = "green")) + 
  theme_minimal() +
  theme(panel.background = element_rect(fill = "#F0F0F0", color = NA), plot.title = element_text(hjust = 0.5)) +
  labs(title = "Alcohol vs Volatile Acidity",x = "Volatile Acidity",y = "Alcohol",color = "Quality")

# Scatter plot:Alcohol vs Density with Quality
ggplot(poor_exc, aes(x = density, y = alcohol, color = quality)) +
  geom_point(alpha = 0.7, size = 3) +
  scale_color_manual(values = c("Poor" = "red","Excellent" = "green")) + 
  theme_minimal() +
  theme(panel.background = element_rect(fill = "#F0F0F0", color = NA), plot.title = element_text(hjust = 0.5)) +
  labs(title = "Alcohol vs Density",x = "Density",y = "Alcohol",color="Quality")

#----------------D------------------------
#alcohol vs residual sugar
ggplot(train_set, aes(x = alcohol, y = residual.sugar)) +
  geom_point(alpha = 0.5, color = "blue") +  
  geom_smooth(method = "lm", color = "red", se = TRUE) +  
  labs(title = "Scatterplot of Residual Sugar vs Alcohol",
       x = "Alcohol",
       y = "Residual Sugar") +
  theme_minimal()
...
#-------------------D---------------------------
#Histogram for RS
ggplot(train_set, aes(x = residual.sugar, fill = type)) + 
  geom_histogram(bins = 40, alpha = 0.5, color = "black", position = "identity") + 
  labs(x = "Chlorides", y = "Count") +
  theme_minimal() +
  scale_fill_manual(values = c("red" = "red", "white" = "yellow"))  # Adjust colors as desired

#Histogram for chlorides
ggplot(train_set, aes(x = chlorides, fill = type)) + 
  geom_histogram(bins = 40, alpha = 0.5, color = "black", position = "identity") + 
  labs(x = "Chlorides", y = "Count") +
  theme_minimal() +
  scale_fill_manual(values = c("red" = "red", "white" = "yellow"))  
...
ggplot(train_set, aes(x = alcohol, fill = type)) + 
  geom_histogram(bins = 30, color = "black", position = "identity", alpha = 0.6) + 
  labs(x = "Alcohol", y = "Count", fill = "Wine Type") +
  theme_minimal() +
  scale_fill_manual(values = c("red" = "#D32F2F", "white" = "#FFEB3B")) 
summary(train_set$alcohol)
